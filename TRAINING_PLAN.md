...
One dedicated lift session per week, standalone (never paired with a climbing or hangboard day), **built to fit inside a 1-hour session — a real, hard constraint, not a nice-to-have.** Rebuilt around four constraints together: only 1 session/week available, no resistance bands, a stated goal of training toward a front lever, and the equipment actually on hand (squat rack, pull-up bar, barbell + plates to 335lb, flat bench). Because there's only one session/week to work with, this is no longer phase-gated like the rest of the plan — it runs the same progression year-round, independent of the climbing periodization cycle.

| Exercise | Sets × Reps | Why it's here |
|---|---|---|
| Front lever progression | 3 sets, hold 3-10s, 3min rest | Direct work toward the stated goal — trained as timed holds through the standard tuck → advanced tuck → straddle → full progression, not a weight/rep lift. Also loads the same hollow-body/anti-extension core pattern a hanging leg raise would, so it replaces that slot rather than adding to it. Kept the full evidence-recommended 3min rest (see below) by trimming a set instead of the rest interval, since this is the actual stated goal. |
