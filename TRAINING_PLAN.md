...
**Rest timer defaults (one preset per exercise, not a menu of choices — evidence-anchored, not guessed):** 180s for the front lever, pull-up, squat, and OHP; 90s for inverted row; 60s for wrist curl. ACSM guidance is a 2-3min minimum for heavy compound work, more approaching max effort; a 2024 Bayesian meta-analysis and a Sports Medicine systematic review both found rest intervals over 60s consistently outperform shorter rests for strength outcomes, with resting only 60s instead of 2-3min measured to cost 10-20% of next-set load capacity. Front lever holds specifically call for a similar ~3min floor per the progression sources above. Inverted row/wrist curl use the standard shorter accessory-work rest interval since they're not max-effort targets.
